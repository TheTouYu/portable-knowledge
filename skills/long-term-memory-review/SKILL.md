...
追加一项**跨项目检查**：目标项目有没有 `federation.json` 注册其他项目？会话里有没有真正调用过
`pkc federation-search`（注意：`pkc --help` 输出里出现命令名**不算**调用）？都没有 → 记
"跨项目记忆未接通"，这是比单项目缺口更高一层的记忆断点。
...
能改，但按风险分层，不是"发现了就无脑改"：

| 级别 | 动作 | 授权 |
|---|---|---|
| L0 只读 | 历史会话分析、PKC 检索、git log/status、读文档 | 随时 |
| L1 文书 | 写复盘报告、更新 docs 权威文档/账本、改技能 SKILL.md（不涉知识树） | 用户明示"优化/修改"后落 + 小步提交 |
| L2 知识树 | 增删 Claim / Authority Ref / bundle apply | 必须走 `pkc-project-operator` 的 hash 审批，复盘者不得自审自批 |
| L3 生产 | 改生产代码 / 游戏文件 / 地图数据 / .gil | 一律禁止；只登记 open-items 或写入继续提示词 |
| 提交 | git add/commit | 用户明示授权；未授权只产出 diff + 建议，不 commit |

复盘技能默认停在 L1 产出建议单；只有用户明确说"改"（如"修改相关技能"）才落到
L1 落地。L2/L3 始终交回用户或目标项目的专业流程，本技能只登记、不越权。
