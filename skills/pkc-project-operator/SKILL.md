...
Never guess conventional paths such as `memory/OPERATING.md`, `memory/CURRENT.md`, or `memory/DECISIONS.md`. Do not read generic README, navigation, source, output, or historical files unless a bounded result explicitly requires one. For a direct business query, stop retrieval when L1 routes to the relevant Topic and L2 supplies enough Claims and boundaries. Use `show-claim` only when exact Authority/evidence status is needed; do not issue adjacent exploratory queries unless the result is insufficient, and explain that expansion. `query` and `knowledge-search` accept `--status any|current|pending_review|not_registered` to filter by Claim-level Authority status: `current` keeps only Claims whose every linked Authority Reference observes the committed baseline, `pending_review` keeps Claims with at least one non-`current` Ref, `not_registered` keeps Claims with no Authority Ref. The filter runs after permission/lifecycle/conflict checks and before pagination, so it narrows only what the caller may already see and never reveals restricted Claims.
...
For Domain Knowledge use only installed high-level PKC `knowledge-plan` operations. For reusable one-command batch intake of one or more new Claims with their Authority Refs, use `knowledge-plan capture --file DRAFT.json`: it runs the same typed pipeline (init, every `add-claim`/`add-authority-ref`, one delta `check`, one `finalize`) and stops with an immutable draft Bundle for exact-hash human review; it never approves, applies, or commits. Its field-level DRAFT contract (top-level/claim/ref fields, required vs optional, controlled vocabularies, minimal JSON + Markdown examples) lives in this skill at `references/capture-draft-format.md` — read it before writing a DRAFT; the equivalent in-repo text is `docs/SEMANTIC-CHANGES.md`「File-driven batch capture」(absent in target projects—do not guess that path). Otherwise use `add-claim` for new knowledge, `revise-claim` to correct/clarify an existing proposition, and `move-topic` to change an existing Topic's Node/path while preserving IDs, Authority links, and history. Every new or semantically expanded Claim needs fact-class coverage: use `add-authority-ref` with the Claim ID returned by the plan before the final delta. **Known tool constraint (0.2.0rc5, genshin-ts 2026-08-30 real hit): a `not_registered` claim (zero Authority Refs) cannot be corrected/expanded with a first Ref in the same plan — `add-authority-ref` after `revise-claim` fails `PLAN_CLAIM_REVISED_NEEDS_REFRESH` (refresh requires an existing Ref), and `refresh --all-stale` would pull every stale Ref in the registry into the plan (operation-count explosion). For that case, either fix upstream (see portable-knowledge `src/portable_knowledge/semantic_plan.py` `add_authority_ref` ~line 580 and delta-check authority forcing ~line 1055, contract test `tests/test_semantic_plan_contract.py` ~line 1728) or defer to a full-library maintenance round; do not loop retries.** It is valid for that purpose-scoped Ref to use a source path already used by another Ref when its Claim link, locator, or fact boundary is distinct; do not omit required coverage merely to avoid a shared path. `refresh-authority-ref PLAN_ID --authority-ref-id ID --reason REASON` only re-approves a changed committed source and deliberately preserves the existing Ref's identity and Claim links—it does not attach a new Claim. For a large batch of stale Refs, `refresh-authority-ref PLAN_ID --all-stale --reason REASON` refreshes every registered stale Ref in one operation (current/missing/dirty Refs are skipped with a per-Ref status, never silently blocked or rewritten). **`--all-stale` is a whole-registry maintenance operation: it pulls every stale Ref into the current plan and can inflate `operation_count` by hundreds (2026-08-30 real hit: 1 → 131). Use it only for an explicit full-library maintenance round (e.g. the 126-ref backlog), never to resolve a single-claim gap; single-claim work uses `--authority-ref-id`, and absent an existing Ref the plan must fail closed as above.** Use `update-authority-ref PLAN_ID --authority-ref-id ID --path NEW_PATH [--locator LOCATOR] --reason REASON` to re-point a Ref to a different file while recomputing its approved hash and preserving the Ref/Claim identity. Use `update-topic PLAN_ID --topic-id ID [--title TITLE] [--summary SUMMARY] [--keyword K ...] [--alias A ...] --reason REASON` to retune Topic metadata (title/summary/keywords/aliases) for retrieval without rewriting Claim bodies. Use `retire-authority-ref PLAN_ID --authority-ref-id ID (--replacement-authority-ref-id ID | --replacement-claim-id ID) --reason REASON` when a Ref no longer applies. These operations must share one plan whenever they form one semantic change. Execute all Claim, Authority Ref, and necessary stale-ref refresh mutations serially; concurrent commands against the same plan are unsupported. Refresh only a Ref whose committed source changed and whose Claim boundary remains valid—never bypass stale Authority or blindly refresh unrelated Refs. After every mutation is complete, run exactly one final `check PLAN_ID --mode delta`, then one successful `finalize PLAN_ID`; do not run exploratory intermediate delta checks or initialize a replacement plan after a command typo. **An applied Bundle is a commit unit (2026-08-16, three real hits): after `bundle-apply --apply` lands, commit that apply's tracked artifacts (authority store, proposals, Bundle files) before starting the next plan.** New plans snapshot authority from the committed baseline (git HEAD); uncommitted applied maintenance triggers `PLAN_AUTHORITY_MAINTENANCE_UNCOMMITTED` / `PLAN_WORKTREE_DRIFT` / `PLAN_STALE_BASELINE`. Do not bypass the guard; follow its guidance: abandon → commit (scoped, intent-labelled) → rebuild the plan on the new baseline. Several Bundles from one batch may share one scoped commit, but the message must name each Bundle intent. If full staged preflight reveals a non-current Ref that could not have been established earlier, keep the same plan, review the exact diagnostic, perform only the justified refresh, and rerun the final delta before retrying finalize. For batch work, one hash-review gate may cover several independent Bundles at once (each with bundle_id + content_hash + change summary + suggested commit message), then approve/apply each individually — never merge Bundles; `bundle-apply` returns a `commit_suggestion` (git_add paths + message) that is the scoped commit checklist for the commit-unit rule. `finalize` also surfaces non-blocking `PLAN_INTENT_OVERLAP` warnings when another Bundle already shares the intent, so orphan/duplicate drafts are visible before apply. **PLAN_* recovery recipe (F3, 2026-08-16):** when `PLAN_WORKTREE_DRIFT` / `PLAN_STALE_BASELINE` / `PLAN_AUTHORITY_MAINTENANCE_UNCOMMITTED` / `PLAN_WORKTREE_SNAPSHOT_DRIFT` blocks a plan, do not fight the guard: (1) read the error's `recommended_action`; (2) if the cause is an applied-but-uncommitted maintenance Bundle, either commit it (scoped, intent-labelled) then `abandon` and `init` on the new baseline, or — when you explicitly accept the current working tree as authority and no later drift is expected — `init --baseline worktree` (pins the working-tree authority hash; later drift still fails closed with `PLAN_WORKTREE_SNAPSHOT_DRIFT`); (3) if the committed baseline simply advanced, `rebase PLAN_ID --reason ...` (keeps operations) or abandon/re-init (replays them); (4) re-run the operations, one final delta check, finalize, and re-verify. `knowledge-plan capture --file DRAFT.json` is the reusable file-driven entry for batch Claims+Refs (R4 satisfied); a markdown `--from-file` variant is a possible later enhancement.
...
## 大规模维护实操模式（Large-scale maintenance mode）

Real-world maintenance sessions (96 stale refs reviewed, 90 refreshes/corrections, 24 claim retunes, 68/68 topic retrieval wiring) prove that large batches behave differently from single edits. Follow these empirical rules:

1. **易变临时文件的瞬态错误**：一个计划操作在 root 下并发存在 `tmp-*.ts` 等易变文件时，若遇到 `No such file or directory .../tmp-*` 这类 `FileNotFoundError`，做有限重试（1–2 次），不要当作计划失败。新版本用稀疏 overlay 只复制权威路径，从根因上消除大部分此类瞬态错误。
2. **ref 指向用户未提交的 dirty 文件**：`PLAN_AUTHORITY_WORKTREE_DIRTY` 意味着"该 Authority 路径有未提交内容"。这是用户职责——暂缓这条 ref，不要硬刷、不要替用户提交、不要把 dirty 工作树当基线。批量维护时该 ref 应被跳过并记录 `skipped_dirty` 状态，而不是让整批失败。
3. **finalized 但未 apply 的 plan 可恢复**：已 finalize 但未 approve/apply 的 plan，`rebase` 会在新基线重新锚定并保留 operations 与 claim 身份（plan 回到 open，旧 bundle 失效清理），不要急着 abandon。一旦有 approval/apply 记录则真正不可变，只能 abandon + 重 capture（会重生成 claim ID，身份成本高）。恢复前先确认是否可避免。
4. **评估门接线时机**：应在全部评测用例可绿之后再接线。有已知失败用例时，full preflight 现在只对 `affected_by` 与 plan 相交的用例做阻塞判定（与 delta 一致），无关用例失败只告警不卡 finalize；用例级 `blocking: false` 只告警；`finalize --defer-evaluation <case-id>` 可显式跳过。用例 `affected_by` 建议收到 topic 级，避免 node 级过宽误伤无关 plan。
5. **检索调优杠杆 = topic keywords/aliases**：标题权重最高（title 1.7 / summary 1.2 / identity 含 keywords+aliases）。调优用 `update-topic`（改 title/summary/keywords/aliases，走 proposal+bundle+apply），不要在正文反复 revise claim 来迁就检索。
6. **改 ref 指向文件用 `update-authority-ref`**：旧路径仍含相关内容时，也可用 `refresh` 兜底；新路径已承接规则时用 `update-authority-ref --ref-id --path [--locator]` 重定向并重算 approved_hash。批量刷新用 `refresh-authority-ref --all-stale --reason`，一次收敛全部 stale ref。

一条维护 plan 内串行完成 Claim、Authority Ref 与必要的 stale refresh，全部 mutation 完成后只做一次最终 delta check 再 finalize；`bundle-approve`/`bundle-apply` 默认 dry-run，必须显式加 `--apply` 才会落盘。apply 后运行 `rebuild`、`validate`、`tree` 与 `git diff --check`。
