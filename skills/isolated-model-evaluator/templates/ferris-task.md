...
用 genshin-model-studio 的 gms 组件 API（`window.gms.part` 米制世界坐标命令）在浏览器页面建一个**物理正确**的摩天轮（双环 + 轮辐 + 支架 + 横杆 + 绳索 + 座舱，完整受力链、零悬空、零重叠、座舱竖直、吊舱 60° 均布）。

## 知识入口
- 技能 `/home/h/.agents/skills/model-build-test/SKILL.md`：**§8 方法论 API 化建模**（命名组件/link/verify/point/touches + §8.5 工作节奏与调试模式）+ §7 组件语义
- 设计基准 `/home/h/genshin-model-studio/benchmark/METHODOLOGY.md`「连接点核验清单」一节（受力链结构与尺寸）
