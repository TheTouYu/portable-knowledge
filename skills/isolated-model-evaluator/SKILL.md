...
- **必须给**：任务目标、用户教学/设计依据（原文或精神）、工作基（副本/输入文件路径）、验收标准、安全边界（禁止项）、知识入口（规则文档位置 + 要加载的核心技能列表）、**项目内可用的现成工具/脚本清单**（CLI 命令 + 一行用途）
- **不要给**：实现细节、具体步骤、坐标/ID/推导结论——这些让子代理自己从规则文档和技能里查出来
- **为什么**：①详细方案是主模型从规则+技能推导的，子代理用同样的材料能自己推导，还给方案会限制它发挥（可能发现更好的做法）；②主模型方案若有错，子代理照做会把错误放大；③"告诉它规则文档在哪里、有哪些核心技能"比"告诉它怎么做"更有价值；④工具清单防止子代理不知道现成 CLI 而自写解析/探测脚本——实测教训（2026-08-10 魔方重构 eval-main）：子代理思考中反复"write a quick probe script"探 JSON/二进制结构、猜字段布局，事后才在技能文档里发现 `parse --json` 已有完整数据（"The parse --json should have full data"）；同轮 eval-split2 762 次 bash 调用中 308 次（40%）是手写 python 探结构。任务文件开头列一行"已有工具：…（用途）"，此类浪费整体消失
...
### 任务文件编写经验（2026-08-11 等角螺线 V4/V5 复盘落地，trace 证据见 /tmp/task-file-lessons-spiral-v4-v5.md）

- **一次派活只换一类变量**：全新算法/几何 + 多处变更拆小，或显式给验证预算；V4（4 处变更 + 技能缺口）900s 超时被掐在收尾，V5（3 参数微调）同样 880s 正常收尾。
- **验收标准写断言式**（阈值+单位），不要定性描述（"或视觉均匀"给了解释空间）；子代理会把验收数字 1:1 翻译成自检断言，零歧义。
- **派活前跑基线，把已知失败写进任务文件**：任务含"现有测试不得变红 / npm build 必须通过"类验收时，派活前先在当前工作树跑一遍 `npm run build` 和可能受影响的测试，把**已知失败清单**（文件+原因+与本次改动无关）写进任务文件。否则子代理会自证基线花掉 ~30 个调用、7 次 `git stash`、8 次 build（2026-08-12 tabBar 任务实测：约 10 分钟/25 分钟预算，其中 routing 测试根因考古 18 个调用）。判定 pre-existing 的快捷方式：`git diff HEAD -- <file>` 无输出（工作树=HEAD）且 `git show HEAD:<file>` 可见缺陷 → 已证明，无需 stash 复跑 build。
- **给验证预算，防过度考古**：写明"root diff 只需确认 rootPresenceStable + 期望 root 集合变化；不要逐字节解释工具内部记录、不要 grep 工具源码"——V4 曾为证明"登记 ±1 不删内容"考古 16 个调用（超时最大单项）。
- **指明基线文件"直接改，别重写"**：有上一轮产物时列文件清单；无基线时在任务文件重复"工具参数以 production-workflow/技能 references 为准，禁止 grep 源码"（技能正文有但子代理未必读到）。
- **易变事实（ID 占用/SHA）标注"以实时为准"并写明"任务里的 ID 表是历史快照，发现不符不是任务写错，照常推进"**——V5 子代理靠此正确处理了用户重导实体导致的 ID 漂移，零浪费。
- **timeout 按任务档位给**：全新算法型 900s 偏紧（V4 两轮贴着上限），参数微调型够用；评估器可显式传 --timeout。
